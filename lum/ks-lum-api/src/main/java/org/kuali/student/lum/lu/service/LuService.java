...
/**
 * <b>IMPORTANT:</b> This service contract is currently under development. If
 * you are planning to implement the Kuali Student System or parts thereof,
 * <b>please do not consider this service to be final!</b> Consult this page for
 * status before making any plans that rely on specific implementations of these
 * services.</p>
 * 
 * <h3><a name="KSDOC-ServiceDescriptions-Description"></a>Description</h3>
 * 
 * <p>
 * The Learning Unit (LU) Service provides management of Canonical Learning
 * Units (CLUs) and Learning Unit Instances (LUIs). This includes the
 * development and approval of new Learning Units and changes to existing
 * Learning Units. A Learning Unit (LU) is any learning-related activity that
 * needs to be tracked by the institution or the learner. Learning units include
 * the traditional curriculum of courses and degrees, professional and extension
 * programs, as well as non-academic activities such as leadership development
 * and service learning. All credit and non-credit learning activities, whether
 * traditional or non-traditional, are Learning Units.
 * </p>
 * 
 * <p>
 * The LU service is broad and covers many aspects of learning either directly
 * or by reference from other services. The LU Service focuses on the creation
 * of the inventory of CLUs that comprise an institution's offering, including:
 * </p>
 * <ul>
 ...
 * 
 * <p>
 * The service includes operations for LUIs, although these are expected to
 * evolve with later KS releases for enrollment and scheduling.
 * </p>
 * 
 ...
 * 
 * <p>
 * <em>Types and Configuration</em><br>
 * The following type concepts are part of configuration and not managed through
 * service operations ::
 * </p>
 * <ul>
 * <li>LuType - overall type of the Learning Unit (for example, course, academic
 * course, exam, program and so forth). Specifics will be determined as part of
 * reference implementation</li>
 * <li>LuLuRelationType - relation type between CLUs and other CLUs and between
 * LUIs and other LUIs (for example, contains, precedes, and so forth)</li>
 * <li>ResultOptionUsageType - expected usage type for the result option (for
 * example, final course grade, program certification)</li>
 * 
 * <li>LuDocRelationType - relation type between an LU and document (for
 * example, proposal rationale, syllabus, reading list, and so forth). This is
 * distinct from what the document self-identifies as.</li>
 * <li>LuStatementType - type or use of statement (for example, academic
 * readiness)</li>
 * <li>LuReqComponentType - type of component used in statements (for example,
 * countLrd, avgGpaLrd)</li>
 * </ul>
 * 
 * 
 * <p>
 * Many of these types are constrained by other types. Management of these
 * constraints is also part of configuration and not managed through service
 * operations:
 * </p>
 * <ul>
 * <li>LuLuRelationType, LrScaleType and LuStatementType are constrained by
 * LuType</li>
 * 
 ...
 * 
 * <p>
 * <em>Behavior and Interaction</em>
 * </p>
 * <ul>
 * <li>Retrieval operations do not require authorization. This needs to be
 * confirmed with use cases to determine if this is the case.</li>
 * <li>Relationships are unidirectional. For example, one CLU precedes another.</li>
 * <li>Only learning objective identifiers are returned from the relevant LU
 * operations. Information on learning objectives is retrieved from the Learning
 * Objective service.</li>
 * 
 * <li>Only result component identifiers are referenced within the result
 * options. Information on the result component and associated values are
 * retrieved from the Learning Result Catalog service.</li>
 * </ul>
 * 
 * 
 ...
 * 
 * <ul>
 * <li><b>Learning Unit Types (LUTs)</b> provide the basic structure or template
 * for creating the CLU. Depending on the type, there may be different
 * information associated with the CLU. Examples of LUT include Course, Program,
 * Project, and Assessment. The granularity or LuType depends on the
 * implementation.</li>
 * <li><b>Canonical Learning Units (CLUs)</b> are the inventory of LU for an
 * institution, the collection of LU that have been defined. These have
 * generally been reviewed and approved through an approval process. There is a
 * status (for example, active, inactive, retired) associated with the canonical
 * learning unit. Canonical LUs are often approved for a range of Academic Time
 * Periods. There are several ways to compose CLUs into complex structures. LUIs
 * inherit the composition of the CLU.
 * <ul>
 * 
 * <li>CluClu Relations can be created to connect CLUs. The relation has a
 * LuLuRelationType which is constrained by the LUT. This construct supports
 * composite CLUs. For example, a Chemistry class that is comprised of a lecture
 * and a lab. CluClu Relations are used to define a series of CLUs in which all
 * courses must be completed before credit is awarded.</li>
 * <li>CLU Sets are collections of CLUs. They can be dynamic (based on a query)
 * or simple enumerated lists. CLU sets are unordered sets of CLUs.</li>
 * </ul>
 * </li>
 * <li><b>Learning Unit Instances (LUIs)</b> are the specific offerings
 * (occurrences) of a Canonical LU, typically during a specific Academic Time
 * Period (ATP), although in some instances there may not be associated calendar
 * dates (for example, online courses).</li>
 * <li><b>Learning Objectives (LOs)</b> have a many-to-many relationship with
 * CLUs. A single CLU has multiple learning objectives and the same learning
 * objective may be related to several CLUs. Learning Objectives are the
 * intended outcome(s) for the LU.
 * <ul>
 * 
 * <li>There is a separate Learning Objective (LO) service that acts as a
 * catalog service to manage the various LOs that may be related to CLU.</li>
 * <li>LOs may also be related directly to a student, which is not part of this
 * service. The current strategy is that Learning Objectives related directly to
 * a student would be part of the Learning Plan (LP) which may be subsumed by
 * the Student Service, which remains to be defined.</li>
 * </ul>
 * </li>
 * <li><b>Result Options</b> are wrappers for the result components defined
 * within the Learning Result Catalog service. Each result option is associated
 * with a specific result option usage type. CLUs and LUIs do not attach
 * directly to result options.</li>
 * <li><b>Result Sets</b> are the collections of one or more result options. The
 * options in a set represent one permutation of results that can be offered by
 * a CLU/LUI and achieved by a student. Each Result Set is associated with an LU
 * Type to constrain to which CLUs they may be attached.</li>
 * 
 * <li><b>CLU Results</b> are the association of CLUs to defined result sets.
 * The CLU Results represent the possible sets of learning results available for
 * an LUI instance. The CLU Results instantiated for an LUI in turn represent
 * the available results for a Learning Unit Person Relation (LPR). LU
 * Statements can be evaluated during creation of an LPR in order to determine
 * which results sets are valid for a specific person.</li>
 * <li><b>Documents</b> are the associated various supporting materials that
 * accompany the CLU from inception, through approval, catalog listing,
 * offering, evaluation and finally retiring. The LU service manages the
 * connection to documents but not the actual maintenance and storage of the
 * documents themselves. They have been separated from CluInfo in support of
 * different authorization access.</li>
 * <li><b>Requirement Components</b> are basic building blocks for describing
 * rules logic associated with various levels of LU (LUT, CLU and LUI). Each
 * ReqComponent has a type that is the handshake with the Business Rules
 * Management Service (BRMS) which knows how to translate ket/value pairs sent
 * along with the type into the appropriate rules. For example, a type of
 * LrdTime could represent a pre-requisite condition of having
 * "taken CHEM1A within the last 6 months and received a grade of B or better."</li>
 * <li><b>Learning Unit Statements</b> represent reusable, structured rule logic
 * comprised of requirement components and/or other statements with a single
 * logical operator (for example, AND, OR) to combine. Each statement is limited
 * to one logical operator. The combination of nesting statements and/or
 * requirement components supports the parentheses need for more complex logic.</li>
 * 
 * </ul>
 * 
...
public interface LuService extends DictionaryService, EnumerableService, SearchService {
	...
	/**
	 * Retrieves the list of allowed learning result types for a specified LU
	 * Type
	 * 
	 * @param luTypeKey
	 *            LU Type Key
	...
	/**
	 * Retrieves the list of allowed learning result scale types for a specified
	 * learning unit type and learning result type
	 * 
	 * @param luTypeKey
	 *            LU Type Key
	 * @param lrTypeKey
	 ...
	 * @throws DoesNotExistException
	 *             luTypeKey, lrTypeKey not found
	 * @throws InvalidParameterException
	 *             invalid luTypeKey, lrTypeKey
	 * @throws MissingParameterException
	 *             missing luTypeKey, lrTypeKey
	 * @throws OperationFailedException
	 *             unable to complete request
	 */
	 ...
	 * 
	 * @param luDocRelationTypeKey
	 *            LU document relationship type key
	 * @return LU document relationship type information
	 * @throws DoesNotExistException
	 *             luDocRelationTypeKey not found
	 * @throws InvalidParameterException
	 *             invalid luDocRelationTypeKey
	 * @throws MissingParameterException
	 *             missing luDocRelationTypeKey
	 * @throws OperationFailedException
	 *             unable to complete request
	 */
	 ...
	 * 
	 * @param luStatementTypeKey
	 *            LU statement type identifier
	 * @return LU statement type information
	 * @throws DoesNotExistException
	 *             luStatementTypeKey not found
	 * @throws InvalidParameterException
	 *             invalid luStatementTypeKey
	 * @throws MissingParameterException
	 *             missing luStatementTypeKey
	 * @throws OperationFailedException
	 *             unable to complete request
	 */
	...
	/**
	 * Retrieves the list of LU statement types which are allowed to be used in
	 * an LU statement type.
	 * 
	 * @param luStatementTypeKey
	 *            luStatementTypeKey
	...
	/**
	 * Retrieves the list of requirement component types which are allowed to be
	 * used in an LU statement type.
	 * 
	 * @param luStatementTypeKey
	 *            luStatementTypeKey
	 * @return list of requirement component types
	...
	/**
	 * Validates a luDocRelation. Depending on the value of validationType, this
	 * validation could be limited to tests on just the current object and its
	 * directly contained sub-objects or expanded to perform all tests related
	 * to this object. If an identifier is present for the organization (and/or
	 * one of its contained sub-objects) and a record is found for that
	 ...
	 * @param validationType
	 *            identifier of the extent of validation
	 * @param luDocRelationInfo
	 *            luDocRelation information to be tested.
	 * @return results from performing the validation
	 * @throws DoesNotExistException
	 *             validationTypeKey not found
	 * @throws InvalidParameterException
	 *             invalid validationTypeKey, luDocRelationInfo
	 * @throws MissingParameterException
	 *             missing validationTypeKey, luDocRelationInfo
	 * @throws OperationFailedException
	 *             unable to complete request
	 */
	...
	/**
	 * Validates a ReqComponent. Depending on the value of validationType, this
	 * validation could be limited to tests on just the current object and its
	 * directly contained sub-objects or expanded to perform all tests related
	 * to this object. If an identifier is present for the organization (and/or
	 * one of its contained sub-objects) and a record is found for that
	...
	/**
	 * Validates a LuStatement. Depending on the value of validationType, this
	 * validation could be limited to tests on just the current object and its
	 * directly contained sub-objects or expanded to perform all tests related
	 * to this object. If an identifier is present for the organization (and/or
	 * one of its contained sub-objects) and a record is found for that
	 ...
	 * @param validationType
	 *            identifier of the extent of validation
	 * @param luStatementInfo
	 *            luStatement information to be tested.
	 * @return results from performing the validation
	 * @throws DoesNotExistException
	 *             validationTypeKey not found
	 * @throws InvalidParameterException
	 *             invalid validationTypeKey, luStatementInfo
	 * @throws MissingParameterException
	 *             missing validationTypeKey, luStatementInfo
	 * @throws OperationFailedException
	 *             unable to complete request
	 */
	 ...
	 *            Key of the LU Type
	 * @param luState
	 *            LU State. Value is expected to be constrained to those in the
	 *            luState enumeration.
	 ...
	 * 
	 * @param luiId
	 *            identifier of the first LUI
	 * @param relatedLuiId
	 *            identifier of the second LUI
	 * @return list of LU to LU relation types
	 * @throws DoesNotExistException
	 *             luiId, relatedLuiId not found
	 * @throws InvalidParameterException
	...
	/**
	 * Retrieves the list of LUI information for the LUIs related to the
	 * specified LUI Id with a certain LU to LU relation type
	 ...
	 * @return list of LUI information
	 * @throws DoesNotExistException
	 *             relatedLuiId, luLuRelationType not found
	 * @throws InvalidParameterException
	 ...
	 * LU relation type (getRelatedLuiIdsByLuiId from the other direction)
	 * 
	 * @param relatedLuiId
	 *            identifier of the LUI
	 * @param luLuRelationType
	 ...
	 * @return list of CLU information
	 * @throws DoesNotExistException
	 *             cluId, luLuRelationType not found
	 * @throws InvalidParameterException
	 ...
	 * and LU to LU relation type (getLuisByRelation from the other direction)
	 * 
	 * @param luiId
	 *            identifier of the LUI
	 * @param luLuRelationType
	 ...
	 * LU relation type. (getLuiIdsByRelation from the other direction)
	 * 
	 * @param luiId
	 *            identifier of the LUI
	 * @param luLuRelationType
	...
	/**
	 * Retrieves the relationship information between LUIs given a specific
	 * relation instance
	 * 
	 ...
	 * @throws DoesNotExistException
	 *             One or more cluSetIds not found
	 * @throws InvalidParameterException
	...
	/**
	 * Retrieves the list of LUI identifiers associated with a given learning
	 * objective identifier.
	 * 
	 * @param loId
	 *            Identifier for the learning objective
	...
	/**
	 * Retrieves the list of learning result type keys associated with a given
	 * CLU.
	 * 
	 * @param cluId
	 *            Identifier for the CLU
	...
	/**
	 * Retrieves the list of learning result scale types for a given learning
	 * result type associated with a given CLU.
	 * 
	 * @param cluId
	 *            Identifier for the CLU
	 * @param lrTypeKey
	 *            Identifier of the learning result type
	 * @return List of learning result scale type keys
	 * @throws DoesNotExistException
	 *             cluId, lrTypeKey not found
	 * @throws InvalidParameterException
	 *             invalid cluId, lrTypeKey
	 * @throws MissingParameterException
	 *             missing cluId, lrTypeKey
	 * @throws OperationFailedException
	 *             unable to complete request
	 */
	...
	/**
	 * Retrieves the list of learning result scale type keys for a given
	 * learning result type associated with a given LUI.
	 * 
	 * @param luiId
	 *            Identifier for the LUI
	 * @param lrTypeKey
	 ...
	 * @return List of learning result scale type keys
	 * @throws DoesNotExistException
	 *             luiId, lrTypeKey not found
	 * @throws InvalidParameterException
	 *             invalid luiId, lrTypeKey
	 * @throws MissingParameterException
	 *             missing luiId, lrTypeKey
	 * @throws OperationFailedException
	 *             unable to complete request
	 */
	 ...
	 * 
	 * @param luDocRelationId
	 *            luDocRelation identifier
	 * @return luDocRelation information
	 * @throws DoesNotExistException
	 *             luDocRelationId not found
	 * @throws InvalidParameterException
	 *             invalid luDocRelationId
	 * @throws MissingParameterException
	 *             luDocRelationId not specified
	 * @throws OperationFailedException
	 *             unable to complete request
	 */
	 ...
	 *             invalid luDocRelationTypeKey
	 * @throws MissingParameterException
	 *             luDocRelationTypeKey not specified
	 * @throws OperationFailedException
	 *             unable to complete request
	 */
	 ...
	 * 
	 * @param luDocRelationIdList
	 *            list of luDocRelation identifiers
	 * @return list of luDocRelation that matches the id list
	 * @throws DoesNotExistException
	 *             luDocRelationId not found
	 * @throws InvalidParameterException
	 *             invalid luDocRelationIdList
	 * @throws MissingParameterException
	 *             luDocRelationIdList not specified
	 * @throws OperationFailedException
	 *             unable to complete request
	 */
	 ...
	 * @throws DoesNotExistException
	 *             luStatementId not found
	 * @throws InvalidParameterException
	 *             invalid luStatementId
	 * @throws MissingParameterException
	 *             luStatementId not specified
	 * @throws OperationFailedException
	 *             unable to complete request
	 */
	 ...
	 *             invalid luStatementTypeKey
	 * @throws MissingParameterException
	 *             luStatementTypeKey not specified
	 * @throws OperationFailedException
	 *             unable to complete request
	 */
	 ...
	 * 
	 * @param reqComponentId
	 *            reqComponent identifier
	...
	/**
	 * Retrieves a list of required components by a list of required component
	 * identifiers
	 * 
	 * @param reqComponentIdList
	 *            List of reqComponent identifiers
	...
	/**
	 * Retrieves a list of Requirement Components that have some sort of
	 * reference to a particular CLU. Note: The reference may not be direct, but
	 * through an intermediate object definition.
	 * 
	 * @param cluId
	 *            clu identifier
	 * @return A list of requirementComponents
	...
	/**
	 * Retrieves a list of Clus that use a particular requirement component.
	 * Note: The reference may not be direct, but through an intermediate object
	 * definition (ex. statements)
	 * 
	 * @param reqComponentId
	 *            requirement component identifier
	 ...
	 * 
	 * @param cluId
	 *            identifier of the CLU for the LUI being created
	 * @param atpKey
	 *            identifier of the academic time period for the LUI being
	 *            created
	 * @param luiInfo
	 ...
	 * 
	 * @param luiId
	 *            identifier for the LUI to be updated
	 * @param luiInfo
	 ...
	 *            identifier for the LUI to be updated
	 * @param luState
	 *            New state for LUI. Value is expected to be constrained to
	 *            those in the luState enumeration.
	 * @return the updated LUI information
	 * @throws DataValidationErrorException
	 *             New state not valid for existing state of LUI
	 * @throws DoesNotExistException
	 *             luiId, luState not found
	 * @throws InvalidParameterException
	 ...
	 * 
	 * @param cluId
	 *            identifier of the first LUI in the relationship
	 * @param relatedCluId
	 ...
	 *            identifier of the first LUI in the relationship
	 * @param relatedLuiId
	 *            identifier of the second LUI in the relationship to be related
	 *            to
	 * @param luLuRelationTypeKey
	 *            the LU to LU relationship type of the relationship
	 ...
	 * 
	 * @param luiLuiRelationId
	 *            identifier of the LUI to LUI relation to update
	 * @param luiLuiRelationInfo
	 *            changed information about the relationship between the two
	 *            LUIs
	 ...
	 * 
	 * @param cluSetName
	 *            name of the CLU set to be created
	 * @param cluSetInfo
	 ...
	 *             CLU set is dynamically determined and clu ids or clu set ids
	 *             are present
	 */
	 ...
	 * 
	 * @param loId
	 *            identifier of learning objective to add to the CLU
	 * @param cluId
	 *            identifier of the CLU
	 * @return status of the operation (success or failure)
	 * @throws AlreadyExistsException
	 *             loId is already associated with the cluId
	 * @throws DoesNotExistException
	 *             loId, cluId not found
	 * @throws InvalidParameterException
	 *             invalid loId, cluId
	 * @throws MissingParameterException
	 *             missing loId, cluId
	 ...
	 * 
	 * @param loId
	 *            identifier of learning objective to remove from the CLU
	 * @param cluId
	 *            identifier of the CLU
	 * @return status of the operation (success or failure)
	 * @throws DependentObjectsExistException
	 *             the learning objective is referenced in one or more LUIs for
	 *             this CLU
	 * @throws DoesNotExistException
	 *             loId, cluId not found
	 * @throws InvalidParameterException
	 ...
	 * 
	 * @param loId
	 *            identifier of learning objective to add to the LUI
	 * @param luiId
	 *            identifier of the LUI
	 * @return status of the operation (success or failure)
	 * @throws AlreadyExistsException
	 *             loId is already associated with the luiId
	 * @throws DoesNotExistException
	 *             loId, luiId not found
	 * @throws InvalidParameterException
	 *             invalid loId, luiId
	 * @throws MissingParameterException
	 *             missing loId, luiId
	 ...
	 *             authorization failure
	 * @throws UnsupportedActionException
	 *             loId is not associated with the parent CLU of the LUI
	 */
	 ...
	 * 
	 * @param loId
	 *            identifier of learning objective to remove from the LUI
	 * @param luiId
	 *            identifier of the LUI
	 * @return status of the operation (success or failure)
	 * @throws DoesNotExistException
	 ...
	 * 
	 * @param lrScaleTypeKey
	 *            identifier of learning result scale to add to the CLU
	 * @param lrTypeKey
	 *            identifier of the learning result type
	 * @param cluId
	 *            identifier of the CLU
	 * @return status of the operation (success or failure)
	 * @throws AlreadyExistsException
	 *             lrScaleTypeKey, lrTypeKey combination is already associated
	 *             with the cluId
	 * @throws DoesNotExistException
	 *             lrScaleTypeKey, lrTypeKey, cluId not found
	 * @throws InvalidParameterException
	 *             invalid lrScaleTypeKey, lrTypeKey, cluId
	 * @throws MissingParameterException
	 *             missing lrScaleTypeKey, lrTypeKey, cluId
	 * @throws OperationFailedException
	 *             unable to complete request
	 * @throws PermissionDeniedException
	 *             authorization failure
	 * @throws UnsupportedActionException
	 *             lrScaleTypeKey, lrTypeKey combination is not associated with
	 *             the parent LU Type of the CLU
	 */
	 ...
	 * 
	 * @param lrScaleTypeKey
	 *            identifier of learning result scale type to remove from the
	 *            CLU
	 * @param lrTypeKey
	 *            identifier of the learning result type
	 * @param cluId
	 ...
	 * @return status of the operation (success or failure)
	 * @throws DependentObjectsExistException
	 *             lrcScaleTypeKey, lrTypeKey combination is referenced in one
	 *             or more LUIs for this CLU
	 * @throws DoesNotExistException
	 *             lrcScaleTypeKey, lrTypeKey, cluId not found
	 * @throws InvalidParameterException
	 *             invalid lrcScaleTypeKey, lrTypeKey, cluId
	 * @throws MissingParameterException
	 *             missing lrcScaleTypeKey, lrTypeKey, cluId
	 * @throws OperationFailedException
	 *             unable to complete request
	 * @throws PermissionDeniedException
	 *             authorization failure
	 ...
	 * 
	 * @param lrScaleTypeKey
	 *            identifier of learning result scale type to add to the LUI
	 * @param lrTypeKey
	 *            identifier of the learning result type
	 * @param luiId
	 *            identifier of the LUI
	 ...
	 *             with the luiId
	 * @throws DoesNotExistException
	 *             lrScaleTypeKey, lrTypeKey, luiId not found
	 * @throws InvalidParameterException
	 *             invalid lrScaleTypeKey, lrTypeKey, luiId
	 * @throws MissingParameterException
	 *             missing lrScaleTypeKey, lrTypeKey, luiId
	 * @throws OperationFailedException
	 *             unable to complete request
	 * @throws PermissionDeniedException
	 *             authorization failure
	 ...
	 * 
	 * @param luDocRelationType
	 *            identifier of the type of luDocRelation
	 * @param documentId
	 *            document identifier
	 * @param cluId
	 *            the identifierof the clu to relate to
	 * @param luDocRelationInfo
	 *            information about the luDocRelation
	 * @return information about the newly created luDocRelation
	 * @throws AlreadyExistsException
	 *             luDocRelation already exists
	 * @throws DataValidationErrorException
	 *             One or more values invalid for this operation
	 * @throws DoesNotExistException
	 *             cluId or luDocRelationTypeKey or documentId not found
	 * @throws InvalidParameterException
	 *             invalid cluId or luDocRelationTypeKey or documentId or
	 *             luDocRelationInfo
	 * @throws MissingParameterException
	 *             missing cluId or luDocRelationTypeKey or documentId or
	 *             luDocRelationInfo
	 * @throws OperationFailedException
	 *             unable to complete request
	 * @throws PermissionDeniedException
	 ...
	 * 
	 * @param luDocRelationId
	 *            identifier of the luDocRelation to be updated
	 * @param luDocRelationInfo
	 *            information about the luDocRelation to be updated
	 * @return the updated luDocRelation information
	 * @throws DataValidationErrorException
	 *             One or more values invalid for this operation
	 * @throws DoesNotExistException
	 *             luDocRelation not found
	 * @throws InvalidParameterException
	 *             invalid luDocRelationId, luDocRelationInfo
	 * @throws MissingParameterException
	 *             missing luDocRelationId, luDocRelationInfo
	 * @throws OperationFailedException
	 *             unable to complete request
	 * @throws PermissionDeniedException
	 *             authorization failure
	 ...
	 * 
	 * @param luDocRelationId
	 *            identifier of the luDocRelation to delete
	 * @return status of the operation (success or failure)
	 * @throws DoesNotExistException
	 *             luDocRelationId not found
	 * @throws InvalidParameterException
	 ...
	 *            identifier of the type of requirement component
	 * @param reqComponentInfo
	 ...
	 * 
	 * @param reqComponentId
	 *            identifier of the requirement component to be updated
	 * @param reqComponentInfo
	 ...
	 * 
	 * @param luStatementType
	 *            identifier of the type of LU statement
	 * @param luStatementInfo
	 *            information about the LU statement
	 * @return information about the newly created LU statement
	 * @throws AlreadyExistsException
	 *             LU statement already exists
	 * @throws DataValidationErrorException
	 *             One or more values invalid for this operation
	 * @throws DoesNotExistException
	 *             luStatementType not found
	 * @throws InvalidParameterException
	 *             invalid luStatementType, luStatementInfo
	 * @throws MissingParameterException
	 *             missing luStatementType, luStatementInfo
	 * @throws OperationFailedException
	 *             unable to complete request
	 * @throws PermissionDeniedException
	 *             authorization failure
	 ...
	 * 
	 * @param luStatementId
	 *            identifier of the LU statement to be updated
	 * @param luStatementInfo
	 *            information about the LU statement to be updated
	 * @return the updated LU statement information
	 * @throws CircularReferenceException
	 *             included LU statement references the current LU statement
	 * @throws DataValidationErrorException
	 *             One or more values invalid for this operation
	 * @throws DoesNotExistException
	 *             luStatement not found
	 * @throws InvalidParameterException
	 *             invalid luStatementId, luStatementInfo
	 * @throws MissingParameterException
	 *             missing luStatementId, luStatementInfo
	 * @throws OperationFailedException
	 *             unable to complete request
	 * @throws PermissionDeniedException
	 *             authorization failure
	 ...
	 * 
	 * @param luStatementId
	 *            identifier of the LU Statement to delete
	 * @return status of the operation (success or failure)
	 * @throws DoesNotExistException
	 *             luStatementId not found
	 * @throws InvalidParameterException
	 ...
	 * @return status
	 * @throws AlreadyExistsException
	 *             connection between clu and statement already exists
	 * @throws DoesNotExistException
	 *             cluId, luStatementId not found
	 * @throws InvalidParameterException
	 *             invalid cluId or luStatementId
	 * @throws MissingParameterException
	 *             cluId or statementId not specified
	 * @throws OperationFailedException
	 *             unable to complete request
	 * @throws PermissionDeniedException
	 *             authorization failure
	 ...
	 * @return Status
	 * @throws DoesNotExistException
	 *             cluId or luStatementId, attachment not found
	 * @throws InvalidParameterException
	 *             invalid cluId or luStatementId
	 * @throws MissingParameterException
	 *             cluId or luStatementId not specified
	 * @throws OperationFailedException
	 *             unable to complete request
	 * @throws PermissionDeniedException
	 *             authorization failure
