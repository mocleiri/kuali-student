...
public class CreditCourseProposalAssembler implements Assembler<Data, Void> {
	// TODO make sure that cluclurelation version indicators are carried over on
	// retrieval
	...
	private CreditCourseHelper getCourse(CreditCourseProposalInfoHelper proposal) throws AssemblyException, DoesNotExistException, InvalidParameterException, MissingParameterException, OperationFailedException {
		...
		// FIXME wilj: figure out how to retrieve the version codes and cross
		// listings
		...
		// Retrieve related clus of type kuali.lu.relation.type.co-located and
		...
		for (CluCluRelationInfo cluRel : cluClus) {
			if (cluRel.getType().equals(JOINT_RELATION_TYPE)) {
				...
				CreditCourseJointsHelper joint = CreditCourseJointsHelper.wrap(new Data());
	...
	private void addFormats(CreditCourseHelper course, CluInfoHierarchy cluHierarchy) throws DoesNotExistException, InvalidParameterException, MissingParameterException, OperationFailedException, AssemblyException {
	...
	private void addActivities(CreditCourseFormatHelper format, CluInfoHierarchy cluHierarchy) throws DoesNotExistException, InvalidParameterException, MissingParameterException, OperationFailedException, AssemblyException {
	...
	private void addCrossListings(CreditCourseHelper course, CluInfoHierarchy cluHierarchy) throws DoesNotExistException, InvalidParameterException, MissingParameterException, OperationFailedException, AssemblyException {
	...
	private void addVersions(CreditCourseHelper course, CluInfoHierarchy cluHierarchy) throws DoesNotExistException, InvalidParameterException, MissingParameterException, OperationFailedException, AssemblyException {
	...
	private CreditCourseProposalInfoHelper getProposal(String id) throws DoesNotExistException, InvalidParameterException, MissingParameterException, OperationFailedException {
	...
	public Metadata getMetadata(String type, String state) throws AssemblyException {
		// TODO overriding the specified type isn't a good thing, but in other
		// cases the type needs to be specified by the caller
		// this needs to be filtered, but not sure how to do that if an
		// assembler needs metadata itself
	...
	private boolean validationFailed(List<ValidationResultInfo> validationResults) {
	...
	private SaveResult<CluInfoHierarchy> saveCourse(CreditCourseHelper course) throws AssemblyException {
		...
		// Uncomment this line when the screen elements for adding join courses
		// is added.
	...
	private void addJoints(String parentCourseId, CreditCourseHelper course) throws AssemblyException {
		try {
			...
			for (Property p : course.getJoints()) {
				CreditCourseJointsHelper joint = CreditCourseJointsHelper.wrap((Data) p.getValue());
				// If user has not entered a joint course on the screen then
				// return back
				...
				if (isCreated(joint.getData())) {
					...
					CluCluRelationInfo result = luService.createCluCluRelation(parentCourseId, joint.getCourseId(), JOINT_RELATION_TYPE, rel);
	...
	private void buildJoints(CluInfo clu, CreditCourseJointsHelper joint) throws AssemblyException {
		try {
			// List<CluInfo> clus = luService.getClusByRelation(parentCourseId,
			// JOINT_RELATION_TYPE);
	...
	private CluInfoHierarchy buildCluInfoHiearchy(CreditCourseHelper course) throws AssemblyException {
		...
		// FIXME wilj: temp check for id, client needs to enforce modification
		// flags once we get the basics working
		if (course.getId() == null) {
			...
			// FIXME wilj: forcing update for now, until client enforces
			// modification flags
	...
	// TODO rewrite alternate identifier code to create separate properties per
	...
	// Map<String, CluIdentifierInfo> ids = new HashMap<String,
	// CluIdentifierInfo>();
	...
	// }
	//		
	// // anything left in the ids map is something that was deleted by the
	// client
	...
	private void buildActivityUpdates(CluInfoHierarchy formatHierarchy, CreditCourseFormatHelper format) throws AssemblyException {
		if (format.getActivities() == null) {
			return;
		}
	...
	private void buildCrossListingsUpdates(CluInfoHierarchy courseHierarchy, CreditCourseHelper course) throws AssemblyException {
		...
		if (isModified(course.getData())) {
			// clear the list because the screen should have all loaded all
			// these AltIdentifiers and
			// they may have been modified by the user and will be populated in
			// for() below.
