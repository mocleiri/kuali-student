...
public class CommentQueryHelper {
	...
	private static MessageDataObject makeMessageDataObject(CommentInfo commentInfo) {
		...
		messageDataObject.setFrom(KsapFrameworkServiceLocator.getUserSessionHelper().getName(
				commentInfo.getAttributeValue(CommentConstants.CREATED_BY_USER_ATTRIBUTE_NAME)));
	...
	private static List<CommentDataObject> getComments(String messageId) {
		...
		for (CommentInfo ci : commentInfos) {
			...
			commentDataObject.setFrom(KsapFrameworkServiceLocator.getUserSessionHelper().getName(
					ci.getAttributeValue(CommentConstants.CREATED_BY_USER_ATTRIBUTE_NAME)));
