...
create table KSA.KSSA_ALLOCATION (ID number(19,0) not null, AMNT number(19,2), IS_LOCKED char(1 char), ACNT_ID_FK varchar2(45 char), TRN_ID_1_FK number(19,0), TRN_ID_2_FK number(19,0), primary key (ID));
...
create table KSA.KSSA_CURRENCY (ID number(19,0) not null, CREATOR_ID varchar2(45 char), DESCRIPTION varchar2(2000 char), EDITOR_ID varchar2(45 char), LAST_UPDATE timestamp, NAME varchar2(100 char), ISO varchar2(10 char), primary key (ID));
...
create table KSA.KSSA_INFORMATION (TYPE varchar2(31 char) not null, ID number(19,0) not null, ACCESS_LEVEL number(10,0), CREATION_DATE timestamp, CREATOR_ID varchar2(45 char), EDITOR_ID varchar2(45 char), EFFECTIVE_DATE timestamp, EXPIRATION_DATE timestamp, LAST_UPDATE timestamp, RESP_ENTITY varchar2(45 char), SEVERITY number(10,0), TEXT varchar2(4000 char), ACNT_ID_FK varchar2(45 char), TRN_ID_FK number(19,0), FLAG_TYPE_ID_FK number(19,0), NEXT_ID number(19,0), PREV_ID number(19,0), primary key (ID));
...
create table KSA.KSSA_TRANSACTION (TYPE varchar2(31 char) not null, ID number(19,0) not null, ALLOCATED number(19,2), AMNT number(19,2), EFFECTIVE_DATE timestamp, EXTN_ID varchar2(45 char), GL_ENTRY_GENERATED char(1 char), IS_INTERNAL_TRN char(1 char), LEDGER_DATE timestamp, LOCKED_ALLOCATED number(19,2), NATIVE_AMNT number(19,2), ORIG_DATE timestamp, REFUND_RULE varchar2(2000 char), RESP_ENTITY varchar2(45 char), STATEMENT_TXT varchar2(100 char), IS_DEFERRED char(1 char), IS_GL_OVERRIDDEN char(1 char), IS_REFUNDABLE char(1 char), DEFER_ID number(19,0), EXPIRATION_DATE timestamp, CLEAR_DATE timestamp, ACNT_ID_FK varchar2(45 char), CURRENCY_ID_FK number(19,0), DOCUMENT_ID_FK number(19,0), ROLLUP_ID_FK number(19,0), TRANSACTION_TYPE_ID_FK varchar2(20 char), TRANSACTION_TYPE_SUB_CODE_FK number(10,0), primary key (ID));
create table KSA.KSSA_TRANSACTION_TYPE (TYPE varchar2(31 char) not null, ID varchar2(20 char) not null, SUB_CODE number(10,0) not null, CREATOR_ID varchar2(45 char), DEF_TRN_TXT varchar2(100 char), EDITOR_ID varchar2(45 char), END_DATE timestamp, LAST_UPDATE timestamp, START_DATE timestamp, AUTH_TXT varchar2(1000 char), CLEAR_PERIOD number(10,0), REFUND_RULE varchar2(2000 char), PRIORITY number(10,0), DEF_ROLLUP_ID_FK number(19,0), primary key (ID, SUB_CODE));
