...
@Configuration
@Import({ModularSchemaExportConfig.class, ModularDataExportConfig.class, LiquibaseModelProviderConfig.class})
public class DbExportConfig {
    ...
    protected final static String SYNC_REQUEST_FILTER_EXPRESSION_KEY = "sync.filter";
     ...
     */
    protected final static String SYNC_REQUEST_FILTER_EXPRESSION_DEFAULT = ".*";
    ...
    public SyncFilesExecutable syncFilesExecutable() {
        ...
        for (String prefix : prefixes) {
            String commitLocation = SpringUtils.getProperty(env, SYNC_COMMIT_PATH_KEY, null);

            String sourceLocation = SpringUtils.getProperty(env, SYNC_REQUEST_SOURCE_DIR_KEY, null);
            String destinationLocation = SpringUtils.getProperty(env, SYNC_REQUEST_DESTINATION_DIR_KEY, null);
            ...
            if(sourceLocation != null) {
                ...
                String filterExpression = SpringUtils.getProperty(env, SYNC_REQUEST_FILTER_EXPRESSION_KEY, SYNC_REQUEST_FILTER_EXPRESSION_DEFAULT);

                // build a scanner with no excludes (3rd argument)
                SimpleScanner scanner = new SimpleScanner(sourceDir, Arrays.asList(filterExpression), null);
