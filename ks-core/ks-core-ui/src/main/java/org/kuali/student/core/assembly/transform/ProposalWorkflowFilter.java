...
public class ProposalWorkflowFilter extends AbstractDataFilter implements MetadataFilter{
    ...
    public void applyOutboundDataFilter(Data data, Metadata metadata,
        ...
        if (TransformFilterAction.SAVE == properties.get(TransformFilter.FILTER_ACTION)){
            ...
            if (proposalInfo.getId() == null){
                ...
                Map<String, String> proposalAttributesMap = (Map<String, String>) properties.get(ProposalWorkflowFilter.PROPOSAL_ATTRIBUTES);

                if (proposalAttributesMap != null) {
                    for (String key : proposalAttributesMap.keySet()) {
                        proposalInfo.getAttributes().add(new AttributeInfo(key, proposalAttributesMap.get(key)));
                    }
                }
	 ...
	 * @param workflowDocumentActionsService
	 */
