...
public class OrgPositionRestrictionInfoAdminLookupableImpl extends LookupableImpl
{
	private static final Logger LOG = Logger.getLogger(OrgPositionRestrictionInfoAdminLookupableImpl.class);
	private transient OrganizationService organizationService;
	private static final long serialVersionUID = 1L;

	@Override
	protected List<OrgPositionRestrictionInfo> getSearchResults(LookupForm lookupForm, Map<String, String> fieldValues, boolean unbounded)
	{
		...
		try
		{
			List<OrgPositionRestrictionInfo> list = this.getOrganizationService().searchForOrgPositionRestrictions(qBuilder.build(), getContextInfo());
			return list;
		}
		catch (Exception ex) {
