...
public class OrgInfoAdminInquirableImpl extends InquirableImpl
{
	private static final Logger LOG = Logger.getLogger(OrgInfoAdminInquirableImpl.class);
	private transient OrganizationService organizationService;
	private final static String PRIMARY_KEY = "id";
	private static final long serialVersionUID = 1L;

	@Override
	public OrgInfo retrieveDataObject(Map<String, String> parameters)
	{
		String key = parameters.get(PRIMARY_KEY);
		try
		{
			OrgInfo info = this.getOrganizationService().getOrg(key, getContextInfo());
			return info;
		}
		catch (Exception ex) {
