...
public class OrgInfoAdminLookupableImpl extends LookupableImpl
{
	private static final Logger LOG = Logger.getLogger(OrgInfoAdminLookupableImpl.class);
	private transient OrganizationService organizationService;
	private static final long serialVersionUID = 1L;

	@Override
	protected List<OrgInfo> getSearchResults(LookupForm lookupForm, Map<String, String> fieldValues, boolean unbounded)
	{
		...
		try
		{
			List<OrgInfo> list = this.getOrganizationService().searchForOrgs(qBuilder.build(), getContextInfo());
			return list;
		}
		catch (Exception ex) {
