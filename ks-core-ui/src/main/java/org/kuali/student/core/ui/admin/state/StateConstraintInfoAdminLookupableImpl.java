...
public class StateConstraintInfoAdminLookupableImpl extends LookupableImpl
{
	private static final Logger LOG = Logger.getLogger(StateConstraintInfoAdminLookupableImpl.class);
	private transient StateService stateService;
	private static final long serialVersionUID = 1L;

	@Override
	protected List<StateConstraintInfo> getSearchResults(LookupForm lookupForm, Map<String, String> fieldValues, boolean unbounded)
	{
		...
		try
		{
			List<StateConstraintInfo> list = this.getStateService().searchForStateConstraints(qBuilder.build(), getContextInfo());
			return list;
		}
		catch (Exception ex) {
