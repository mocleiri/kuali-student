...
public class OrgHierarchyInfoAdminLookupableImpl extends LookupableImpl
{
	private static final Logger LOG = Logger.getLogger(OrgHierarchyInfoAdminLookupableImpl.class);
	private transient OrganizationService organizationService;
	private static final long serialVersionUID = 1L;

	@Override
	protected List<OrgHierarchyInfo> getSearchResults(LookupForm lookupForm, Map<String, String> fieldValues, boolean unbounded)
	{
		...
		try
		{
			List<OrgHierarchyInfo> list = this.getOrganizationService().searchForOrgHierarchies(qBuilder.build(), getContextInfo());
			return list;
		}
		catch (Exception ex) {
