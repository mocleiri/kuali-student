...
public class StatePropagationInfoAdminInquirableImpl extends InquirableImpl
{
	private static final Logger LOG = Logger.getLogger(StatePropagationInfoAdminInquirableImpl.class);
	private transient StateService stateService;
	private final static String PRIMARY_KEY = "id";
	private static final long serialVersionUID = 1L;

	@Override
	public StatePropagationInfo retrieveDataObject(Map<String, String> parameters)
	{
		String key = parameters.get(PRIMARY_KEY);
		try
		{
			StatePropagationInfo info = this.getStateService().getStatePropagation(key, getContextInfo());
			return info;
		}
		catch (Exception ex) {
