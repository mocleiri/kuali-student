...
public class OrgOrgRelationInfoAdminLookupableImpl extends LookupableImpl
{
	private static final Logger LOG = Logger.getLogger(OrgOrgRelationInfoAdminLookupableImpl.class);
	private transient OrganizationService organizationService;
	private static final long serialVersionUID = 1L;

	@Override
	protected List<OrgOrgRelationInfo> getSearchResults(LookupForm lookupForm, Map<String, String> fieldValues, boolean unbounded)
	{
		...
		try
		{
			List<OrgOrgRelationInfo> list = this.getOrganizationService().searchForOrgOrgRelations(qBuilder.build(), getContextInfo());
			return list;
		}
		catch (Exception ex) {
