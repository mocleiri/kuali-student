...
public class StateChangeInfoAdminLookupableImpl extends LookupableImpl
{
	private static final Logger LOG = Logger.getLogger(StateChangeInfoAdminLookupableImpl.class);
	private transient StateService stateService;
	private static final long serialVersionUID = 1L;

	@Override
	protected List<StateChangeInfo> getSearchResults(LookupForm lookupForm, Map<String, String> fieldValues, boolean unbounded)
	{
		...
		try
		{
			List<StateChangeInfo> list = this.getStateService().searchForStateChanges(qBuilder.build(), getContextInfo());
			return list;
		}
		catch (Exception ex) {
