...
/**
 * A property resource configurer that resolves placeholders in bean property values of context definitions. It
 ...
 * </pre>
 * 
 * PropertyPlaceholderConfigurer checks simple property values, lists, maps, props, and bean names in bean references.
 * Furthermore, placeholder values can also cross-reference other placeholders, like:
 * 
 * <pre class="code">
 * rootPath=myrootdir
 ...
 * </pre>
 * 
 * In contrast to PropertyOverrideConfigurer, this configurer allows to fill in explicit placeholders in context
 * definitions. Therefore, the original definition cannot specify any default values for such bean properties, and the
 * placeholder properties file is supposed to contain an entry for each defined placeholder.
 * 
 * <p>
 * If a configurer cannot resolve a placeholder, a BeanDefinitionStoreException will be thrown. If you want to check
 * against multiple properties files, specify multiple resources via the "locations" setting. You can also define
 * multiple PropertyPlaceholderConfigurers, each with its <i>own</i> placeholder syntax.
 * 
 * <p>
 * Default property values can be defined via "properties", to make overriding definitions in properties files optional.
 * A configurer will also check against system properties (e.g. "user.dir") if it cannot resolve a placeholder with any
 * of the specified properties. This can be customized via "systemPropertiesMode".
 * 
 * <p>
 * Note that the context definition <i>is</i> aware of being incomplete; this is immediately obvious to users when
 * looking at the XML definition file. Hence, placeholders have to be resolved; any desired defaults have to be defined
 * as placeholder values as well (for example in a default properties file).
 * 
 * <p>
 ...
 */
public class PPC2 extends PropertyResourceConfigurer implements BeanNameAware, BeanFactoryAware {
	...
	private static final Constants constants = new Constants(MyPropertyPlaceholderConfigurer.class);
	 ...
	 * <code>null</code> if no such special character should be processed as a value separator. The default is ":".
	 */
	 ...
	 * <p>
	 * Default is "false": An exception will be thrown if a placeholder fails to resolve. Switch this flag to "true" in
	 * order to preserve the placeholder String as-is in such a case, leaving it up to other placeholder configurers to
	 * resolve it.
	 */
	...
	/**
	 * Set a value that should be treated as <code>null</code> when resolved as a placeholder value: e.g. "" (empty
	 * String) or "null".
	 * <p>
	 * Note that this will only apply to full property values, not to parts of concatenated values.
	 * <p>
	 * By default, no such null value is defined. This means that there is no way to express <code>null</code> as a
	 * property value unless you explictly map a corresponding value here.
	 */
	...
	/**
	 * Only necessary to check that we're not parsing our own bean definition, to avoid failing on unresolvable
	 * placeholders in properties file locations. The latter case can happen with placeholders for system properties in
	 * resource locations.
	 * 
	 * @see #setLocations
	 ...
	 *            the system properties mode, according to the constants in this class
	 ...
	 *            the placeholders that have already been visited during the current resolution attempt (ignored in this
	 *            version of the code)
	 * @deprecated as of Spring 3.0, in favor of using {@link #resolvePlaceholder} with
	 *             {@link org.springframework.util.PropertyPlaceholderHelper}. Only retained for compatibility with
	 *             Spring 2.5 extensions.
	 */
	@Deprecated
	...
	private class PropertyPlaceholderConfigurerResolver implements PlaceholderResolver {
		...
		public String resolvePlaceholder(String placeholderName) {
			return PPC2.this.resolvePlaceholder(placeholderName, props, systemPropertiesMode);
		}
	}
