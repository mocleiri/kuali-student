...
public class BrmFeeManagementServiceImpl extends GenericPersistenceService implements BrmFeeManagementService {
     ...
     */
    @Override
    public void writeSessionLog(String logLevel, String message, BrmContext context) {

        FeeManagementSession session = getRequiredAttribute(context, FM_SESSION_VAR_NAME);

        FeeManagementSessionLogLevel logLevelValue = StringUtils.isNotBlank(logLevel) ?
                EnumUtils.findById(FeeManagementSessionLogLevel.class, logLevel) : FeeManagementSessionLogLevel.INFO;

        fmService.writeSessionLog(session.getId(), logLevelValue, message);
    }
