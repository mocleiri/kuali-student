...
public class FeeAssessmentServiceImpl extends GenericPersistenceService implements FeeAssessmentService, BeanFactoryAware {
		
	/**
	 * The logger.
	 */
	private static final Log logger = LogFactory.getLog(FeeAssessmentServiceImpl.class);
	...
	public List<LearningPeriod> findLearningPeriods(Date dateFrom, Date dateTo) {
		// Create a Query:
		Query query = em.createQuery("select from LearningPeriod where dateFrom >= :dateFrom and dateTo <= :dateTo", LearningPeriod.class)
