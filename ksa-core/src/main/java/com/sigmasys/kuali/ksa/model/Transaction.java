...
public abstract class Transaction extends AccountIdAware implements Identifiable {
    ...
    /**
     * The identifier of the transaction as generated by another external system.
     * If the external system does not generate a transaction identifier, this will be the same as the internal transaction identifier.
     * For credits, this might take the form of the credit card authorization, etc.
     */
     ...
     * It is also the date that payment application will use to order the transactions.
     * This date is also used as the defining date for a transaction if its GL code has multiple effective accounts.
     */
    protected Date effectiveDate;

    /**
     * This is the date that the transaction originated, so, for example, housing might bill on a certain day, but upload its transactions only once a week.
     * The purpose of this date is purely for tracking transactions from external systems.
     */
    protected Date originationDate;

    /**
     * Period for which the transaction is recognized on the general ledger.
     * By default, this is set to the effectiveDate, unless overridden.
     */
    protected Date recognitionDate;


    /**
     * This is the value of the transaction in the system currency.
     * This is the number that is used as the core value of the transaction. All calculations are performed against this number.
     */
    protected BigDecimal amount;

    /**
     * This is the native amount of the transaction in the currency in which it was created.
     * For most transactions, this will be identical to the localAmount, as most transactions occur in the local currency.
     */
    protected BigDecimal nativeAmount;

    /**
     * This is the identifier for the currency that is used in nativeAmount. Most often, it will be set to the same value as the system currency.
     * This is a 3-letter ISO4217 code. For example, USD, GBP, EUR.
     */
    protected Currency currency;

    /**
     * This flag, when true, indicates that the transaction is 'internal', and will not, in most cases, be displayed to the customer.
     * In most cases, internal transactions will be allocated and locked against an equivalent transaction.
     * An example of this would be if a charge is incorrectly applied to an account, a reversal transaction would be created,
     * the transactions could then be allocated together, and marked as internal.
     */
    ...
    /**
     * This value is set to a value (up to the maximum of localAmount) and is the value of the transaction that has been allocated,
     * and may not be reallocated by the payment application routine. Most commonly, the entire transaction will be locked, by setting the lockedAllocationAmount to the localValue.
     * Schools will also likely lock their allocations of transactions before the current period of time (for example, if an allocation was made with a payment in a previous year,
     * it will not be allowed to be de-allocated by the system.
     */
    ...
    /**
     * The text for the transaction that will be displayed as a summary of the transaction on statements.
     * For example "Bookstore Purchase"  If this string is null during instantiation, the default text will
     ...
     * This information will be different depending on the type of transaction. The information will be available when a user "drills down" into a transaction.
     * It will contain information relating to the charge or payment, such as the contact details for the department who generated a charge,
     * and other information as made available by the system. For example, a charge from the bookstore may list the items purchased.
     * The document referenced will be an XML document, allowing flexible creation of different document types by the institutions themselves.
     * Some basic document types will be defined by the project.
     */
