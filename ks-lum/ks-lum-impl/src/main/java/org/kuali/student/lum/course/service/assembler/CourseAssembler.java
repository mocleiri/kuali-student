...
public class CourseAssembler implements BOAssembler<CourseInfo, CluInfo> {
    ...
    public CourseInfo assemble(CluInfo clu, CourseInfo courseInfo,
        ...
        course.setCourseTitle(clu.getOfficialIdentifier().getLongNames().get(0).getValue());
...
//        course.setDepartment(clu.getPrimaryAdminOrg().getOrgId());
    ...
    public BaseDTOAssemblyNode<CourseInfo, CluInfo> disassemble(
        ...
        IntlValueInfo nameInfo = new IntlValueInfo();
        nameInfo.setValue(course.getCourseTitle());
        ...
        for(CourseVariationInfo variation:course.getVariations()){
            ...
            IntlValueInfo name = new IntlValueInfo();
            name.setValue(variation.getVariationTitle());
            name.setLocale("EN");
    ...
    private List<BaseDTOAssemblyNode<?, ?>> disassembleCreditOutcomes(CourseInfo course, CluInfo clu, List<CluResultInfo> currentCluResults, NodeOperation operation, ContextInfo contextInfo) throws AssemblyException, NumberFormatException {
        ...
        if(NodeOperation.DELETE!=operation){
            ...
            try{
                try {
                    rsltComps.addAll(lrcService.getResultComponentIdsByResultComponentType(CourseAssemblerConstants.COURSE_RESULT_COMP_TYPE_CREDIT_FIXED, contextInfo));
                } catch (DoesNotExistException e) {}
                try {
                    rsltComps.addAll(lrcService.getResultComponentIdsByResultComponentType(CourseAssemblerConstants.COURSE_RESULT_COMP_TYPE_CREDIT_MULTIPLE, contextInfo));
                } catch (DoesNotExistException e) {}
                try {
                    rsltComps.addAll(lrcService.getResultComponentIdsByResultComponentType(CourseAssemblerConstants.COURSE_RESULT_COMP_TYPE_CREDIT_VARIABLE, contextInfo));
                } catch (DoesNotExistException e) {}
    ...
    private List<BaseDTOAssemblyNode<?, ?>> disassembleLos(String cluId,  CourseInfo course, NodeOperation operation,ContextInfo contextInfo) throws AssemblyException {
        ...
        for(LoDisplayInfo loDisplay : R1R2ConverterUtil.convertLists(course.getCourseSpecificLOs(), LoDisplayInfo.class)){
            ...
            if (NodeOperation.CREATE == operation
                ...
                loDisplay.getLoInfo().setStateKey(course.getState());
        ...
        if (cluIdents != null) {
            for (CluIdentifierInfo cluIdent : cluIdents) {
                if (cluIdent.getType() != null &&
                    ...
                    variation.setVariationTitle(cluIdent.getLongNames().get(0).getValue());
