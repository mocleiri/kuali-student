...
#DURATION COUNT
  ...
  element(:duration_count_count) { |b| b.textarea(id: "KS-DurationTimeQuantity-Field_control") }
...
#FINAL EXAM
  ...
  element(:final_exam_rationale) { |b| b.textarea(name: 'document.newMaintainableObject.dataObject.finalExamRationale') }
...
#OUTCOME
  ...
  action(:credit_value) { |outcome_level, b| b.textarea(id: "creditValue_line#{outcome_level}_control") }
...
#COURSE FORMAT(S)
  ...
  element(:contacted_hours) { |format_level,activity_level,b| b.textarea(id: /line#{format_level-1}_line#{activity_level-1}_control/, name: /contactHours.unitQuantity$/) }
  ...
  action(:duration_count) { |format_level,activity_level,b| b.textarea(id: "KS-Logistics-Format-DurationTimeQuantity-Field_line#{format_level-1}_line#{activity_level-1}_control") }
  ...
  action(:class_size) { |format_level,activity_level,b| b.textarea(id:/line#{format_level-1}_line#{activity_level-1}_control/, name: /defaultEnrollmentEstimate$/) }
  ...
  element(:contacted_hours_added) { |format_level,activity_level,b| b.textarea(name: "document.newMaintainableObject.dataObject.formats[#{format_level-1}].activities[#{activity_level-1}].contactHours.unitQuantity").value }
  ...
  action(:duration_count_added) { |format_level,activity_level,b| b.textarea(name: "document.newMaintainableObject.dataObject.formats[#{format_level-1}].activities[#{activity_level-1}].duration.timeQuantity").value }
  action(:class_size_added) { |format_level,activity_level,b| b.textarea(name: "document.newMaintainableObject.dataObject.formats[#{format_level-1}].activities[#{activity_level-1}].defaultEnrollmentEstimate").value }
