class AdminComments < BasePage
  ...
  element(:comment_list_header) { |b| b.frm.div(id: 'KS-Comment_list_Header') }
  value(:comment_list_header_text) { |b| b.frm.div(id: 'KS-Comment_list_Header').text } #TODO: parse out number of comments

  #element(:comment_list_div) { |b| b.frm.div(id: 'u1bcxmtx') }
  element(:comments_list) { |b| b.frm.divs(id: /^KS-collection-rowId_line\d+$/) }

  def comment_div_by_text (comment_text)
  ...
  element(:close) { |b| b.frm.button(id: 'commentsLightBoxContents-cancel') }
end