...
public class TestSqlOrganizer {
    ...
    public class TypeMapping {
        ...
        public Boolean isValid() {
            return type.toString().equals(decode(typeKey));
        }
    }

    public enum ControlMappingStatus {
        ...
        private ControlMappingStatus(String stringVal) {
    ...
    public void testSkipFile() throws IOException {
        ...
        assertTrue(skipSqlOrganization(statement));
    }
    ...
    private void processSqlStatements(String sqlFile) throws IOException {
        ...
        String statement = sqlReader.getSql(reader);
        if (!skipSqlOrganization(statement)) {
            int i;
            for (i=0; statement != null; statement = sqlReader.getSql(reader)) {
                ...
                List<String> tableNames = null;
                    ...
                    // TODO: if update && refernce then consider migration
                    // TODO: might consider a CLEANUP category to run after migration for delete of BOOSTRAP data
                    statementBuckets.get(dataType).get(module).append(statement + "\n/\n");

            }
            System.out.println("num of statements in file: " + i);
            splitFile(statementBuckets, filename);
        } else {
            TypeMapping typeMapping = new TypeMapping();
            ControlMappingStatus status = setControlDataType(statement, filename, typeMapping);
            if (status == ControlMappingStatus.VALID) {
                System.out.println("file control comment found.  Moving file to " + typeMapping.getType().toString());
                FileUtils.copyFile(new File(sqlFile), new File(OUTPUT_DIR_PATH + typeMapping.getType().toString() + "//" + getDatabaseModule().getEndsWith() + "//" + filename));
            } else {
                System.out.println("error processing control comment: " + status.toString());
                String newFilePath = OUTPUT_DIR_PATH + DatabaseDataType.EXCEPTION + "//" + getDatabaseModule().getEndsWith() + "//" + filename;
                File newFile = new File(newFilePath);
                FileUtils.copyFile(new File(sqlFile), newFile);
    ...
    private void splitFile(Map<DatabaseDataType, Map<DatabaseModule, StringBuilder>> statementBuckets, String filename) {
        ...
        for (Map.Entry<DatabaseDataType, Map<DatabaseModule, StringBuilder>> typeEntry : typeEntries) {
            ...
            for (Map.Entry<DatabaseModule, StringBuilder> moduleEntry : moduleEntries) {
                if (moduleEntry.getValue().length() > 0) {
                    writeToFile(typeEntry.getKey(), moduleEntry.getKey(), filename, moduleEntry.getValue().toString());
                }
            }
        }
    }

    private ControlMappingStatus setControlDataType(String statement, String filename, TypeMapping typeMapping) {
                ...
                if (kvps.length == 2) {
                    if (kvps[0].equals("KEY1")) {
                        if (!validateKey(filename, kvps[1])) {
                            return ControlMappingStatus.INVALID_KEY;
                        } else {
                            validKey = true;
                        }
                        ...
                        if (typeMapping.getType() == null) {
                            return ControlMappingStatus.TYPE_NOT_FOUND;
                        }
                    } else if (kvps[0].equals("KEY2")) {
                        typeMapping.setTypeKey(kvps[1]);
                    } else {
                        return ControlMappingStatus.INVALID_PROPERTY;
                    }
                } else {
                    return ControlMappingStatus.FORMAT_ERROR;
                }
            }
        } catch (IOException e) {
            typeMapping.setType(DatabaseDataType.EXCEPTION);
            return ControlMappingStatus.IOEXCEPTION;
        }

        if (!validKey) {
            return ControlMappingStatus.MISSING_KEY;
        } else if (!validType) {
            return ControlMappingStatus.MISSING_TYPE;
        } else if (!typeMapping.isValid()) {
            return ControlMappingStatus.INVALID_TYPE_KEY;
        }
        return ControlMappingStatus.VALID;
    }

    private boolean validateKey(String filename, String encoded) {
        if (filename.equals(decode(encoded))) {
            return true;
        } else {
            System.out.println("Control comment likely copied from: " + decode(encoded));
        }
        return false;
    }
    ...
    public void testGetModule() {
        ...
        Assert.assertTrue(DatabaseModule.KSCM.equals(getModule(tableNames,DatabaseModule.KSCM)));
        Assert.assertTrue(DatabaseModule.KSCM.equals(getModule(tableNames, DatabaseModule.KSENR)));

        tableNames.clear();
        tableNames.add("KSXX_TABLE1");
        tableNames.add("KSXX_TABLE2");
        tableNames.add("KSXX_TABLE3");
        Assert.assertTrue(DatabaseModule.KSCORE.equals(getModule(tableNames,DatabaseModule.KSCORE)));
        Assert.assertTrue(DatabaseModule.KSCORE.equals(getModule(tableNames,DatabaseModule.KSENR)));

        tableNames.clear();
        tableNames.add("KRXX_TABLE1");
        tableNames.add("KRXX_TABLE2");
        tableNames.add("KRXX_TABLE3");
        Assert.assertTrue(DatabaseModule.RICE.equals(getModule(tableNames, DatabaseModule.RICE)));
        Assert.assertTrue(DatabaseModule.RICE.equals(getModule(tableNames,DatabaseModule.KSENR)));

        tableNames.clear();
        tableNames.add("KRXX_TABLE1");
        tableNames.add("KSXX_TABLE2");
        Assert.assertTrue(DatabaseModule.EXCEPTION.equals(getModule(tableNames, DatabaseModule.RICE)));
        Assert.assertTrue(DatabaseModule.EXCEPTION.equals(getModule(tableNames, DatabaseModule.KSCORE)));

    }

    private DatabaseModule getModule(List<String> tableNames, DatabaseModule defaultModule) {
    ...
    public void cleanOutputDirs() {
        cleanOutputDir(new File(OUTPUT_DIR_PATH));
        for (DatabaseDataType typeIter : DatabaseDataType.values()) {
            cleanOutputDir(new File(OUTPUT_DIR_PATH + typeIter.toString()));
            for (DatabaseModule moduleIter : DatabaseModule.values()) {
                cleanOutputDir(new File(OUTPUT_DIR_PATH + typeIter.toString() + "\\" + moduleIter.getEndsWith()));
            }
        }
    }

    public void cleanOutputDir(File path) {
        ...
        if (isDirectoryCreated) {
            ...
            deleteDir(path);  // Invoke recursive method
            path.mkdir();
        }
}

    private void writeToFile(DatabaseDataType type, DatabaseModule module, String filename, String content) {

        try {
            File file = new File(OUTPUT_DIR_PATH + type.toString() + "\\" + module.getEndsWith() + "\\" + filename);
    ...
    public void testEncryptions() {
        testEncryption("2013-02-14-KSLU-Add-course-proposal-ref-data-rice.sql");
        testEncryption("2013-06-12-Fix-core-constraint-names.sql");
    ...
    private boolean skipSqlOrganization(String firstLine) {
    ...
    private String cleanStmt(String statement) {
        String cleanStmt = StringUtils.replaceEach(statement, new String[] {"VARCHAR2","varchar2", "Varchar2"}, new String[] {"VARCHAR", "varchar", "Varchar"});
