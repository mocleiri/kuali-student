Feature: SA.ELIG8-8 Permissions for specific users should change depending on the CO
  ...
  @pending
  Scenario: Test that Carol can only view AO Requisites for a CO that is not ENGL
  ...
  @pending
  Scenario: Test that Carol can add a new rule in ENGL AO Requisites
  ...
  @pending
  Scenario: Test that the function buttons work when Carol is editing a rule
  ...
  @pending
  Scenario: Test if Carol can use the functionality on the logic tab
    When I edit the rule that replaced the CO rule in the Student Eligibility & Prerequisite section
