...
//  POA #1: Exclude sections with a null/blankTimeScheduleLinkAbbreviation.
    ...
    /**
     * gets a list of section data from solr xml response
     *
     * @param solrXml
     * @return
     */
    public static List<String> getSectionsFromSolrXML(String solrXml) {
        DefaultXPath solrPath = new DefaultXPath("//doc");
        Document solrDocument = null;
        try {
            solrDocument = newDocument(solrXml);
        } catch (DocumentException e) {
            logger.error(e);
        }
        List sections = solrPath.selectNodes(solrDocument);
        List<String> sectionList = new ArrayList<String>();
        for (Object node : sections) {
            Element section = (Element) node;
            String secondarySectionID = section.selectSingleNode("str").getText();
    ...
    /**
     * gets the section data from solr Xml response
     *
     * @param solrXml
     * @return
     */
    public static String getSectionDataFromSolrXML(String solrXml) {
        DefaultXPath solrPath = new DefaultXPath("//str[@name='section.data']");
        Document solrDocument = null;
        try {
            solrDocument = newDocument(solrXml);
        } catch (DocumentException e) {
