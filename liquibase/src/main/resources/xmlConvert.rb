...
@indent = "\t"
@impexDirName = '/home/andy/src/kualiWorkspace/ks-1.3/ks-cfg-dbs/ks-embedded-db/src/main/impex'
...
impexDir.each do |dirEntry|
	if dirEntry.include? '.xml' and dirEntry != 'schema.xml'
		...
		outputFile.puts @indent + '<changeLog id="' + tableName + '-initial-data" author="alubbers">'
		
		writeInsertTags(absoluteFile, outputFile)
		
		outputFile.puts @indent + '</changeLog>'
