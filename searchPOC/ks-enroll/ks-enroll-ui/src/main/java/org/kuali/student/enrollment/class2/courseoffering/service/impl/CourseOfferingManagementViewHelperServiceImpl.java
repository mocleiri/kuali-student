...
public class CourseOfferingManagementViewHelperServiceImpl extends CO_AO_RG_ViewHelperServiceImpl implements CourseOfferingManagementViewHelperService{
    ...
    public void loadCourseOfferingsByTermAndCourseCode(String termId, String courseCode, CourseOfferingManagementForm form) throws Exception {
        ...
        loadSearchedCourseOfferings(termId, courseCode, form);
    ...
    private void loadSearchedCourseOfferings(String termId, String courseCode, CourseOfferingManagementForm form) throws Exception, MissingParameterException, PermissionDeniedException, OperationFailedException {
        QueryByCriteria.Builder qbcBuilder = QueryByCriteria.Builder.create();
        qbcBuilder.setPredicates(PredicateFactory.and(
                PredicateFactory.like("courseOfferingCode", StringUtils.upperCase(courseCode) + "*"),
        ...
        List<CourseOfferingInfo> courseOfferings = getCourseOfferingService().searchForCourseOfferings(criteria, contextInfo);

        long end = System.currentTimeMillis();

        LOG.info("in loadSearchedCourseOfferings, time to run search is: " + (end - start) + "ms.");

        for (CourseOfferingInfo co : courseOfferings) {
            CourseOfferingListSectionWrapper coListWrapper = new CourseOfferingListSectionWrapper();

            coListWrapper.setCourseOfferingCode(co.getCourseOfferingCode());
            coListWrapper.setSubjectArea(co.getSubjectArea());
            coListWrapper.setCourseOfferingId(co.getId());
            coListWrapper.setCourseOfferingDesc(co.getCourseOfferingTitle());
            coListWrapper.setCourseOfferingStateKey(co.getStateKey());
            coListWrapper.setCourseOfferingStateDisplay(getStateInfo(co.getStateKey()).getName());

            coListWrapper.setCourseOfferingCreditOptionKey(co.getCreditOptionId());
            CourseOfferingTransformer courseOfferingTransformer = new CourseOfferingTransformer();
            coListWrapper.setCourseOfferingCreditOptionDisplay(courseOfferingTransformer.getCreditCount(co.getCreditOptionId(), "", null, null, contextInfo));

            coListWrapper.setCourseOfferingGradingOptionKey(co.getGradingOptionId());
            ResultValuesGroupInfo rvgInfo = getLrcService().getResultValuesGroup(co.getGradingOptionId(), contextInfo);
            coListWrapper.setCourseOfferingGradingOptionDisplay(rvgInfo.getName());

            if(!co.getCrossListings().isEmpty()) {
                coListWrapper.setCrossListed(true);
                coListWrapper.setAlternateCOCodes(new ArrayList<String>(co.getCrossListings().size()));
                for(CourseOfferingCrossListingInfo crossCo : co.getCrossListings()) {
                    coListWrapper.getAlternateCOCodes().add(crossCo.getCode());
                }
            }


            List<String> orgIds = co.getUnitsDeploymentOrgIds();
            if(orgIds != null && !orgIds.isEmpty()) {
                coListWrapper.setAdminOrg(CollectionUtils.getCSV(orgIds));
            }
        ...
        LOG.info("time to load all CO data for search: " + (allDone - start) + "ms");
    }
